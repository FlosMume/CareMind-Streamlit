...
# --- ensure a modern SQLite for Chroma on hosts with old sqlite3 ---
try:
    import pysqlite3  # provided by pysqlite3-binary
    import sys
    sys.modules["sqlite3"] = sys.modules.pop("pysqlite3")
except Exception:
    # If the shim isn't available, we proceed; Chroma may later raise a clear error.
    pass
# -------------------------------------------------------------------
...
def _chroma():
    # Import only when needed (avoids failing at module import time)
    from chromadb import PersistentClient
    from chromadb.utils import embedding_functions
    return PersistentClient, embedding_functions

def get_chroma_client(persist_dir: str | Path):
    PersistentClient, _ = _chroma()
    return PersistentClient(path=str(persist_dir))

def get_embed_fn(model_name: str = "sentence-transformers/all-MiniLM-L6-v2"):
